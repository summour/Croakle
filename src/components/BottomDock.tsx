      ...
      {groups.map((group) => {
        ...
        return (
          <button
            ...
            className={`flex-1 flex items-center justify-center py-2.5 px-1 rounded-[24px] transition-all duration-200 min-h-[46px] sm:min-h-[48px] ios-tap relative ${
              isActive
                ? 'bg-gradient-to-b from-[#ffffff] via-[#faf6f0] to-[#f0e7dc] text-[#221c17] shadow-[0_6px_16px_-2px_rgba(0,0,0,0.22),inset_0_1.5px_1.5px_rgba(255,255,255,1)] scale-[1.03]'
                : 'text-[#a89b8d] hover:text-[#f4efe8] hover:bg-white/[0.06]'
            }`}
          >
            ...
            {isActive && (
              <span className="absolute bottom-1 w-1.5 h-1.5 rounded-full bg-[#5f7a61] dark:bg-[#7d9d80] shadow-[0_0_6px_rgba(95,122,97,0.8)]" />
            )}
          </button>
        );
      })}
